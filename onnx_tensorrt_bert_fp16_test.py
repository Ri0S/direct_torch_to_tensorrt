...
if __name__ == '__main__':
    ...
    if '_bert_half.trt' not in os.listdir('.'):
        torch.onnx.export(model, (input_ids, attention_mask, token_type_ids), '_bert_half.onnx', input_names=['input_ids', 'attention_mask', 'token_type_ids'], output_names=['last_hidden_state', 'pooler_output'], export_params=True)

        builder = trt.Builder(logger)
        network = builder.create_network(1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_BATCH))
        parser = trt.OnnxParser(network, logger)
        parser.parse_from_file('_bert_half.onnx')
        ...
        with open('_bert_half.trt', 'wb') as f:
            f.write(serialized_engine)
    else:
        with open('_bert_half.trt', 'rb') as f:
